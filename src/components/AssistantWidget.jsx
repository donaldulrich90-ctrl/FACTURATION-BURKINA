...
export default function AssistantWidget() {
  ...
  const [showHint, setShowHint] = useState(() => {
    try { return !localStorage.getItem('fasomarches_assistant_hint_seen'); } catch { return true; }
  });
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const { reply } = await api.postAssistantMessage(text, history);
      ...
      if (err?.fallback) {
        setMessages((prev) => [
          ...
          {
            role: 'assistant',
            content: 'L\'assistant IA n\'est pas encore configuré. Consultez le Guide utilisateur (GUIDE_ENTREPRISES.pdf) ou contactez votre administrateur pour activer l\'assistant.',
          },
        ]);
      }
    } finally {
  ...
  const handleOpen = () => {
    ...
    if (showHint) {
      setShowHint(false);
      try { localStorage.setItem('fasomarches_assistant_hint_seen', '1'); } catch {}
    }
  };
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-40 flex items-center gap-2">
        {showHint && !open && (
          <div className="bg-white dark:bg-faso-card shadow-lg border border-faso-border rounded-faso-lg px-3 py-2 text-sm text-faso-text-primary animate-pulse">
            <span className="font-medium text-faso-primary">Posez vos questions à l'IA</span>
            <span className="text-faso-text-secondary ml-1">— Je vous guide</span>
          </div>
        )}
        <button
          ...
          title="Assistant IA — Posez vos questions sur la plateforme"
        >
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-40 w-[380px] max-w-[calc(100vw-48px)] h-[520px] max-h-[70vh] bg-white dark:bg-faso-card rounded-faso-lg shadow-xl border border-faso-border flex flex-col overflow-hidden">
          <div className="bg-faso-primary text-white px-4 py-3 flex items-center gap-2">
            <Bot size={22} />
            <div>
              <h3 className="font-bold text-sm">Assistant FasoMarchés</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="space-y-3">
                <p className="text-sm text-faso-text-secondary">
                  Bonjour ! Je suis l'assistant virtuel de FasoMarchés. Je peux vous aider à comprendre la plateforme et vous guider dans son utilisation.
                </p>
            ...
            {error && !loading && (
              <p className="text-xs text-faso-statut-rejete">{error}</p>
            )}
          ...
          <form onSubmit={sendMessage} className="p-3 border-t border-faso-border">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="p-2 bg-faso-primary text-white rounded-faso hover:bg-faso-primary-hover disabled:opacity-50"
              >
                <Send size={18} />
              </button>
            </div>
