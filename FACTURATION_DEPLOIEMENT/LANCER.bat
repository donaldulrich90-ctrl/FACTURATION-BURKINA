...
:: 3) Dependances frontend (installer si manquant ou incomplet)
...
if not exist "node_modules\.bin\concurrently.cmd" (
    echo [OK] Mise a jour des dependances frontend...
    call npm install --silent 2>nul
    if errorlevel 1 call npm install --legacy-peer-deps --silent 2>nul
)
if not exist "node_modules\.bin\concurrently.cmd" (
...
:: 4) Dependances serveur (installer si manquant ou incomplet)
if not exist "server\node_modules" (
...
if not exist "server\node_modules\.bin\nodemon.cmd" (
    echo [OK] Mise a jour des dependances serveur...
    cd server
    call npm install --silent 2>nul
    cd "%ROOT%"
)
...
if not exist "server\.env" (
    ...
    (
        ...
        echo PORT=3001
        echo.
        echo # Assistant IA : decommentez et ajoutez votre cle Groq ^(gratuit sur console.groq.com^)
        echo # GROQ_API_KEY=gsk_votre_cle
    ) > "server\.env"
)
