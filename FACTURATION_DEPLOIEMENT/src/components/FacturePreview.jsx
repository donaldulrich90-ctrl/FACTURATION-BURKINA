...
export default function FacturePreview({
  ...
  const handlePrint = () => {
    ...
    printWrap.style.cssText = 'width:210mm;min-height:297mm;margin:0;padding:0;background:white;';
...
<div
        ref={exportRef}
        id="facture-print"
        className="relative p-12 min-h-[297mm] flex flex-col shrink-0 overflow-x-auto"
      ...
      >
            ...
            <table className="w-full text-sm border-collapse" style={{ tableLayout: 'fixed', width: '100%' }}>
              ...
              <thead>
                <tr className="bg-slate-100 text-slate-700">
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs">Réf</th>
                  <th className="border border-slate-400 p-2 text-left font-semibold text-xs">Désignation</th>
                  <th className="border border-slate-400 p-2 text-left font-semibold text-xs">Marques</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id || idx} className="border-b border-slate-300">
                    <td className="p-2 text-center border border-slate-300 font-mono text-xs break-all" style={{ wordBreak: 'break-all', overflowWrap: 'anywhere' }}>{item.code || idx + 1}</td>
                    <td className="p-2 border border-slate-300 align-top" style={{ wordBreak: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal' }}>
                      <span className="block">{item.designation || '—'}</span>
                    </td>
                    <td className="p-2 border border-slate-300 align-top" style={{ wordBreak: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal' }}>
                      <span className="block">{item.specificationTechnique || '—'}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <table className="w-full text-sm border-collapse" style={{ tableLayout: 'fixed', width: '100%' }}>
              ...
              <thead>
                <tr className="bg-slate-100 text-slate-700">
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs">Réf</th>
                  <th className="border border-slate-400 p-2 text-left font-semibold text-xs">Désignation</th>
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs whitespace-nowrap">Qté min</th>
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs whitespace-nowrap">Qté max</th>
                  <th className="border border-slate-400 p-2 text-right font-semibold text-xs whitespace-nowrap">P.U.</th>
                  <th className="border border-slate-400 p-2 text-right font-semibold text-xs whitespace-nowrap">Total min</th>
                  <th className="border border-slate-400 p-2 text-right font-semibold text-xs whitespace-nowrap">Total max</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="border-b border-slate-300">
                      ...
                      <td className="p-2 text-center border border-slate-300 text-xs">{qMin}</td>
                      <td className="p-2 text-center border border-slate-300 text-xs">{qMax}</td>
                      <td className="p-2 text-right font-mono text-xs border border-slate-300">{formatNombre(pu)}</td>
                      <td className="p-2 text-right font-mono text-xs border border-slate-300">{formatNombre(totalMin)}</td>
                      <td className="p-2 text-right font-mono font-medium text-xs border border-slate-300">{formatNombre(totalMax)}</td>
                    </tr>
                  );
                })}
                {/* Ligne TOTAL : dans tbody pour éviter répétition à l'impression (tfoot se répète sur chaque page) */}
                <tr className="bg-slate-100 border-t-2 border-slate-400">
                  <td colSpan={5} className="p-2 border border-slate-400 font-bold text-xs text-slate-700">TOTAL</td>
                  <td className="p-2 text-right font-mono font-bold text-xs border border-slate-400">{formatNombre(totalMinGlobal)}</td>
                  <td className="p-2 text-right font-mono font-bold text-xs border border-slate-400">{formatNombre(totalMaxGlobal)}</td>
                </tr>
              </tbody>
            </table>
          ) : (
            <table className="w-full text-sm border-collapse" style={{ tableLayout: 'fixed', width: '100%' }}>
              ...
              <thead>
                <tr className="bg-slate-100 text-slate-700">
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs">Réf</th>
                  <th className="border border-slate-400 p-2 text-left font-semibold text-xs">Désignation</th>
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs">Quantité</th>
                  <th className="border border-slate-400 p-2 text-center font-semibold text-xs">Unité</th>
                  <th className="border border-slate-400 p-2 text-right font-semibold text-xs">Prix unitaire</th>
                  <th className="border border-slate-400 p-2 text-right font-semibold text-xs">Total</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="border-b border-slate-300">
                      <td className="p-2 text-center border border-slate-300 font-mono text-xs break-all" style={{ wordBreak: 'break-all', overflowWrap: 'anywhere' }}>{item.code || idx + 1}</td>
                      <td className="p-2 border border-slate-300" style={{ wordBreak: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal' }}>
                        <span className="block">{item.designation || '—'}</span>
                      </td>
                      <td className="p-2 text-center border border-slate-300 text-xs">{qty}</td>
                      <td className="p-2 text-center border border-slate-300 text-xs">{item.unite || 'U'}</td>
                      <td className="p-2 text-right font-mono text-xs border border-slate-300">{formatNombre(pu)}</td>
                      <td className="p-2 text-right font-mono font-medium text-xs border border-slate-300">{formatNombre(total)}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {/* Totaux (hors mode commande et spec_tech : totaux dans le tableau) */}
        {modeFacture !== 'commande' && docType !== 'spec_tech' && (
        <div className="mt-6 flex justify-end">
          <div className="w-80 space-y-1 text-sm">
              <>
                ...
                <div className="flex justify-between text-lg font-bold pt-2 border-t-2 border-slate-400">
        ...
        <div data-facture-zone-admin className="mt-8 p-4 border-2 border-dashed border-slate-400 rounded-lg bg-slate-50">
          {docType === 'spec_tech' ? (
            <p className="text-sm text-slate-600 italic">Document établi pour spécification technique — sans valeur comptable.</p>
          ) : modeFacture === 'commande' ? (
