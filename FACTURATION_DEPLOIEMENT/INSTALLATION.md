...
- **Assistant IA ne répond pas** : ajoutez `GROQ_API_KEY=votre_cle` dans `server/.env` (clé gratuite sur https://console.groq.com/keys). Redémarrez le serveur.
- **Chat équipe ne s'affiche pas** : connectez-vous avec un compte entreprise (gérant ou collaborateur), pas avec admin@plateforme.com. Le chat nécessite le mode API (serveur démarré).
